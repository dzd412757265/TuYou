...
#define SOUND_SWITCH_KEY    @"SoundSwitchKey"
...
@property (nonatomic, assign) SystemSoundID swipeSound;
...
//        self.needVibrateWhenNotChatting = [[[NSUserDefaults standardUserDefaults] objectForKey:STR_BY_SEL(needVibrateWhenNotChatting)] boolValue];
        ...
        [self createSoundWithType:@"swipe" type:@"wav" soundId:&_swipeSound];
        
        //初始状态声音是打开的
        self.soundOff = [[NSUserDefaults standardUserDefaults] boolForKey:SOUND_SWITCH_KEY];
    }
    return self;
}
...
- (void)createSoundWithType:(NSString *)name type:(NSString *)type soundId:(SystemSoundID *)soundId {
...
- (void)playSwipeSoundIfNeed{
    ...
    if (!self.soundOff) {
          AudioServicesPlaySystemSound(_swipeSound);
    }
...
- (void)setSoundOff:(BOOL)soundOff{
    _soundOff = soundOff;
    [[NSUserDefaults standardUserDefaults] setBool:soundOff forKey:SOUND_SWITCH_KEY];
}
